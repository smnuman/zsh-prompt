...
#   PRE_REMOTE=${PRE_REMOTE:-0}
  ...
  if git rev-parse --is-inside-work-tree &>/dev/null; then
    ...
    local REMOTES=$(git_remote_segment)
    # (( $PRE_REMOTE )) && echo "%K{15} ${REMOTES}${BRANCH_NAME}${GIT_STATUS}${ARROWS}${_VS_GIT_}%k" || \
    #                      echo "%K{15} ${BRANCH_NAME}${GIT_STATUS}${ARROWS}${REMOTES}${_VS_GIT_}%k"
...
function git_remote_segment() {
  ...
  if git rev-parse --is-inside-work-tree &>/dev/null; then
    ...
    local segment=""
    [[ -n $github ]] && segment+="%F{blue}${spc}GH:$github%f"
    [[ -n $gitlab ]] && segment+="%F{magenta}${spc}GL:$gitlab%f"
    # echo "%K{15}$segment%k"
...
function set-remote() {
    ...
    (( PRE_REMOTE )) && print -r -- "${A}${B}" || print -r -- "${B}${A}"
}
